...
-(void)viewInit
{
    ...
    thumbView.text = @"1.点击右上角\"设置\"按钮选择声纹类型\n\n2.点击\"训练模型\"按钮，通过训练向服务器注册声纹模型\n\n3.在训练模型界面，如果是固定密码或者是数字密码类型，点击\"开始录音\"按钮并朗读麦克风图标上方文字，朗读完等待1秒后点击\"停止录音\"，重复直到下方显示的数字为5,自由说可随意朗读\n\n4.自由说训练和验证时请放慢朗读节奏以保证成功率\n\n5.训练成功后，可以点击\"识别声纹\"按钮进行验证\n\n6.可以点击\"删除模型\"或者是\"查询模型\"进行删除或是查询声纹模型操作";
